...
<div class="max-w-lg mx-auto bg-white p-6 rounded shadow mt-10">
    ...
    <div class="mb-3 text-sm text-gray-600">
        Prix unitaire (HT) actuel : <span class="font-semibold">{{ number_format($unitPrice, 2, ',', ' ') }} €</span>
    </div>
    ...
    <form action="{{ route('units.purchase') }}" method="POST" id="unitForm" enctype="multipart/form-data">
        ...
        {{-- Totaux dynamiques --}}
        <div class="mb-2">
            <label class="text-sm text-gray-600">TVA ({{ (int)$vatRate }}%)</label>
...
<script>
    ...
    // Values from PHP (controller)
    const UNIT_PRICE = Number(@json($unitPrice));   // HT
    const TVA_RATE   = Number(@json($vatRate)) / 100; // e.g. 20 => 0.20

    function updateTotals(){
        const qty = Math.max(0, parseFloat(quantityInput.value || 0));
        const subtotal = qty * UNIT_PRICE; // HT
