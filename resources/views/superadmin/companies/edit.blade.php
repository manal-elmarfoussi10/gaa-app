...
<div class="px-6 py-6 max-w-7xl mx-auto space-y-6">
  ...
  <form action="{{ route('superadmin.companies.update', $company) }}" method="POST" enctype="multipart/form-data"
    ...
    <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div><label class="block text-sm font-medium text-gray-600 mb-1">Nom *</label>
          <input name="name" required value="{{ old('name',$company->name) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">Nom commercial</label>
          <input name="commercial_name" value="{{ old('commercial_name',$company->commercial_name) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">Téléphone</label>
          <input name="phone" value="{{ old('phone',$company->phone) }}" class="w-full border rounded-lg p-3"></div>
        <div class="md:col-span-3"><label class="block text-sm font-medium text-gray-600 mb-1">Email</label>
          <input type="email" name="email" value="{{ old('email',$company->email) }}" class="w-full border rounded-lg p-3"></div>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-600 mb-1">Adresse</label>
          <input name="address" value="{{ old('address',$company->address) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">Code postal</label>
          <input name="postal_code" value="{{ old('postal_code',$company->postal_code) }}" class="w-full border rounded-lg p-3"></div>
        <div class="md:col-span-3"><label class="block text-sm font-medium text-gray-600 mb-1">Ville</label>
          <input name="city" value="{{ old('city',$company->city) }}" class="w-full border rounded-lg p-3"></div>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div><label class="block text-sm font-medium text-gray-600 mb-1">Forme juridique</label>
          <input name="legal_form" value="{{ old('legal_form',$company->legal_form) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">Capital (EUR)</label>
          <input type="number" step="0.01" name="capital" value="{{ old('capital',$company->capital) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">SIRET</label>
          <input name="siret" value="{{ old('siret',$company->siret) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">N° TVA</label>
          <input name="tva" value="{{ old('tva',$company->tva) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">RCS n°</label>
          <input name="rcs_number" value="{{ old('rcs_number',$company->rcs_number) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">RCS Ville</label>
          <input name="rcs_city" value="{{ old('rcs_city',$company->rcs_city) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">APE</label>
          <input name="ape" value="{{ old('ape',$company->ape) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">Code NAF</label>
          <input name="naf_code" value="{{ old('naf_code',$company->naf_code) }}" class="w-full border rounded-lg p-3"></div>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div><label class="block text-sm font-medium text-gray-600 mb-1">Méthode de paiement</label>
          <input name="payment_method" value="{{ old('payment_method',$company->payment_method) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">IBAN</label>
          <input name="iban" value="{{ old('iban',$company->iban) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">BIC</label>
          <input name="bic" value="{{ old('bic',$company->bic) }}" class="w-full border rounded-lg p-3"></div>
        <div><label class="block text-sm font-medium text-gray-600 mb-1">Taux pénalités (%)</label>
          <input name="penalty_rate" value="{{ old('penalty_rate',$company->penalty_rate) }}" class="w-full border rounded-lg p-3"></div>
      </div>
    </div>
